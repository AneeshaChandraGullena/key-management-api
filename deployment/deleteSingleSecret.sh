...
curl -w "%{http_code}\n" ${CURL_FLAGS} -X DELETE -H "Content-Type: application/json" -H "Authorization: ${TOKEN}" -H "Bluemix-Space: ${SPACE}" -H "Bluemix-Org: ${BLUEMIX_ORG}" \
"${SCHEME}://${KP_API_SERVER_IP}:8990/api/v2/secrets/${SECRET_ID}"
